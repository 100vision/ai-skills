...
class PPTSkillHandler:
    ...
    def render_ppt_from_draft(self, payload_path: str, output_filename: str) -> dict:
        ...
        if not os.path.exists(".engine_cache.json"):
            return {"status": "error", "message": "错误：请先运行 inspect 模式扫描模板，生成引擎映射缓存。"}
            
        with open(".engine_cache.json", "r") as f:
            cache_data = json.load(f)
            self.layout_idx_cache = cache_data.get("engine_cache", {})
            self.current_template = cache_data.get("template_file", "my_template.pptx")
        ...
        prs = Presentation(self.current_template)
    ...
    def _get_ph(self, slide, target_idx):
        for ph in slide.placeholders:
            if ph.placeholder_format.idx == target_idx: return ph
        return None
